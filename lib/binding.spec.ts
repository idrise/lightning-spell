...
describe("LightningSpell", () => {
  it("should allow a custom dictionary to be constructed", () => {
    ...
    expect(lightningSpell.getSuggestions("united stte")).toEqual([
      "united state"
    ]);
    expect(
      lightningSpell.getSuggestions("united stte", { distance: 2 })
    ).toEqual(["united state", "united states"]);
  });
  ...
  it("should work with 0f", () => {
    ...
    expect(lightningSpell.getSuggestions("0f", { maxResults: 8, distance: 2 }))
      .toEqual([
        "of",
        "if",
        "0s",
